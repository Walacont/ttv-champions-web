...
const templates = await db.collection('recurringTrainingTemplates')
  .where('clubId', '==', 'YOUR_CLUB_ID')
  .get();
console.log('Templates:', templates.docs.map(d => d.data()));

const sessions = await db.collection('trainingSessions')
  .where('clubId', '==', 'YOUR_CLUB_ID')
  .where('date', '>=', 'YYYY-MM-DD')
  .get();
console.log('Auto-generated Sessions:', sessions.docs.map(d => d.data()));
```
...
const attendance = await db.collection('attendance')
  ...
  .where('clubId', '==', 'YOUR_CLUB_ID')
  .get();
console.log('Attendance records:', attendance.docs.map(d => ({
...
**Validierung**:
```javascript
const pairings = await db.collection('trainingMatches')
  .doc('SESSION_ID')
  .get();
...
**Validierung**:
```javascript
const session = await db.collection('trainingSessions')
  .doc('SESSION_ID')
  .get();
...
const sessions = await db.collection('trainingSessions')
  .where('date', '==', dateStr)
  .get();
console.log('Sessions for tomorrow:', sessions.docs.map(d => d.data()));
```
