 ...
 * Run with: node find-club-id.cjs
 */
...
async function findClubId() {
  ...
  try {
    ...
    if (clubIds.size === 0) {
      ...
      console.log('\nFühren Sie "node list-collections.cjs" aus, um die Datenbank-Struktur zu sehen.');
    } else {
      ...
      if (clubIds.size === 1) {
        ...
        console.log(`   node debug-match-history.cjs ${theClubId}`);
      }
    }
