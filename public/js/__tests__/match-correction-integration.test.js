...
describe.skipIf(!isLocalSupabaseRunning)('Match Correction Integration', () => {
    ...
    beforeAll(async () => {
        ...
        // Reset DB to seed state (clean up from any previous test runs)
        // 1. Delete all correction requests
        await serviceClient.from('match_requests').delete().not('id', 'is', null);
        // 2. Delete non-seed matches and reset seed matches
        await serviceClient
            .from('matches')
            .delete()
            .not('id', 'in', `(${MATCH1_ID},${MATCH2_ID})`);
        await serviceClient
            .from('matches')
            .update({ is_corrected: false, corrected_by_match_id: null })
            .in('id', [MATCH1_ID, MATCH2_ID]);
        // 3. Reset player profiles to seed values
        await serviceClient
            .from('profiles')
            .update({
                elo_rating: 800,
                ...
                points: 3,
                xp: 16,
                singles_matches_played: 2,
            })
            .in('id', [PLAYER1_ID, PLAYER2_ID]);
        // 4. Delete reversal points_history entries (keep only seed entries)
        await serviceClient.from('points_history').delete().eq('reason', 'Korrektur');
        // 5. Reset H2H stats
        await serviceClient
            ...
            .update({
                ...
                player_b_wins: 1,
                consecutive_wins: 1,
                current_streak_winner_id: PLAYER2_ID,
                suggested_handicap: 0,
                last_winner_id: PLAYER2_ID,
            })
            .eq('player_a_id', PLAYER1_ID)
            .eq('player_b_id', PLAYER2_ID);
    ...
    describe('reverse_match_effects RPC', () => {
        test('reverses Match 1 effects on player profiles', async () => {
            ...
            const { data: after, error: afterErr } = await serviceClient
                .from('profiles')
                .select('id, elo_rating, wins, losses, points, singles_matches_played')
                .in('id', [PLAYER1_ID, PLAYER2_ID]);
