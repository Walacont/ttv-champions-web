...
function initializeDashboard(userData) {
    ...
    loadGlobalLeaderboard(userData);
...
function loadLeaderboard(userData) {
    ...
    leagueNameEl.textContent = `${userLeague}-Liga`;
    ...
    const leaderboardListener = onSnapshot(q, (snapshot) => {
     ...
     unsubscribes.push(leaderboardListener);
}

function loadGlobalLeaderboard(userData) {
    ...
    unsubscribes.push(globalListener);
}

function renderPlayerRow(player, index, currentUserId, container, totalPlayers, isGlobal = false) {
