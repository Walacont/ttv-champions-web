...
function renderReceivedRequests(requests) {
    ...
    const html = requests.map(request => {
        const photoUrl = request.requester_avatar_url || request.requester_photo_url || 'https://placehold.co/64x64/e2e8f0/64748b?text=' + (request.requester_first_name?.[0] || '?');
...
function renderSentRequests(requests) {
    ...
    const html = requests.map(request => {
        const photoUrl = request.addressee_avatar_url || request.addressee_photo_url || 'https://placehold.co/64x64/e2e8f0/64748b?text=' + (request.addressee_first_name?.[0] || '?');
...
function renderFriendsList(friends) {
    ...
    const html = friends.map(friend => {
        const photoUrl = friend.avatar_url || friend.photo_url || 'https://placehold.co/64x64/e2e8f0/64748b?text=' + (friend.first_name?.[0] || '?');
