...
import { formatDateGerman } from './validation-utils.js';
...
function renderFormedPairs() {
  ...
  formedPairs.forEach((pair, index) => {
    ...
    const showMilestoneSelect =
      isMilestoneExercise && (pair.result === 'both_success' || pair.result === 'one_success');
    let milestoneSelect = '';
...
async function confirmPairingAndDistributePoints() {
  ...
  const pairingData = {
    ...
    singlePlayers: singlePlayers.map(sp => {
      ...
      // For milestone exercises, calculate completionCount from selected milestone
      const exerciseMilestones =
        sp.customExercise?.tieredPoints?.milestones || currentExercise?.tieredPoints?.milestones;
