/**
 * Tutorial System - Wiederverwendbare Tutorial-Engine (Supabase Version)
    ...
    async saveTutorialCompletion() {
        ...
        if (supabase && userId && this.options.tutorialKey) {
            try {
                ...
                // Bestehende Daten oder leere Objekte
                const tutorialCompleted = currentData?.tutorial_completed || {};
                const tutorialCompletedAt = currentData?.tutorial_completed_at || {};
                ...
                const { error: updateError } = await supabase
                    ...
                    .update({
                        ...
                        tutorial_completed_at: tutorialCompletedAt
                    })
                    .eq('id', userId);
...
export async function getTutorialStatus(tutorialKey, supabase, userId) {
    ...
    try {
        ...
        if (data && data.tutorial_completed) {
            return data.tutorial_completed[tutorialKey] || false;
        }
    } catch (error) {
