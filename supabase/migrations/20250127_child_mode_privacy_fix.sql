...
BEGIN
    ...
    -- Get all member IDs from the child's club (excluding hidden profiles)
    SELECT ARRAY_AGG(id) INTO v_member_ids
    FROM profiles
    WHERE club_id = v_club_id
    AND COALESCE(privacy_settings->>'searchable', 'global') != 'none';

    -- ============================================
    -- 1. Singles Matches (with privacy filter)
    -- Only show matches where BOTH players allow visibility
    -- ============================================
    ...
    FROM (
        SELECT
            ...
            mat.created_at
        FROM matches mat
        -- Join to get privacy settings for both players
        ...
        -- Privacy check: both players must allow visibility
        AND (
            -- Child is a player in this match - always visible
            mat.player_a_id = v_child_id OR mat.player_b_id = v_child_id
            OR
            -- Both players have visible matches (not 'none')
            (
                COALESCE(pa.privacy_settings->>'matches_visibility', 'global') != 'none'
                AND COALESCE(pb.privacy_settings->>'matches_visibility', 'global') != 'none'
                -- For 'club_only', check if child is in same club
                AND (
                    COALESCE(pa.privacy_settings->>'matches_visibility', 'global') IN ('global', 'followers_only')
                    OR pa.club_id = v_club_id
                )
                AND (
                    COALESCE(pb.privacy_settings->>'matches_visibility', 'global') IN ('global', 'followers_only')
                    OR pb.club_id = v_club_id
                )
            )
        )
    ...
    -- ============================================
    -- 2. Doubles Matches (with privacy filter)
    -- ============================================
    ...
    FROM (
        ...
        -- Privacy check: child is a player OR all players allow visibility
        AND (
            -- Child is a player
            ...
            OR
            -- All players allow visibility
            (
                COALESCE(p1.privacy_settings->>'matches_visibility', 'global') != 'none'
                AND COALESCE(p2.privacy_settings->>'matches_visibility', 'global') != 'none'
                AND COALESCE(p3.privacy_settings->>'matches_visibility', 'global') != 'none'
                AND COALESCE(p4.privacy_settings->>'matches_visibility', 'global') != 'none'
            )
        )
        ORDER BY mat.created_at DESC
        LIMIT p_limit OFFSET p_offset
    ...
    -- ============================================
    -- 3. Activity Events (with privacy filter)
    -- Exclude events from users with searchable='none'
    -- ============================================
    ...
    FROM (
        ...
        -- Privacy: user must not be hidden
        AND COALESCE(p.privacy_settings->>'searchable', 'global') != 'none'
    ...
    -- ============================================
    -- 4. Community Posts (with privacy filter)
    -- ============================================
    ...
    -- ============================================
    -- 5. Community Polls (with privacy filter)
    -- ============================================
