...
const WIDGETS = [
  ...
  { id: 'training-stats', name: '📈 Trainings-Statistiken', description: 'Anwesenheit und Aktivitätsverlauf', default: true },
...
// Training Stats - attendance records
const trainingAttendanceQuery = computed(() => {
...
const trainingAttendance = useCollection(trainingAttendanceQuery)

// Get training dates for stats (last 12 months)
const trainingDates = computed(() => {
  if (!trainingAttendance.value) return []

  const oneYearAgo = new Date()
  oneYearAgo.setFullYear(oneYearAgo.getFullYear() - 1)
  const oneYearAgoStr = oneYearAgo.toISOString().split('T')[0]

  return trainingAttendance.value
...
// Calculate monthly training stats
const trainingMonthlyStats = computed(() => {
  const now = new Date()
...
// GitHub-style heatmap data for training (last 52 weeks)
const trainingHeatmapData = computed(() => {
...
const trainingMonthLabels = ['Jan', 'Feb', 'Mär', 'Apr', 'Mai', 'Jun', 'Jul', 'Aug', 'Sep', 'Okt', 'Nov', 'Dez']
...
<template>
  <div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      <div v-if="isWidgetVisible('training-stats')" class="lg:col-span-3 bg-white p-6 rounded-xl shadow-md">
        <div class="flex items-center justify-between mb-4">
          <h2 class="text-xl font-semibold text-gray-800">📈 Trainings-Statistiken</h2>
        ...
        <div class="grid grid-cols-3 gap-4 mb-6">
          <div class="bg-gradient-to-br from-blue-50 to-indigo-50 p-4 rounded-lg border border-blue-200">
            ...
            <p class="text-3xl font-bold text-blue-900">{{ trainingMonthlyStats.currentMonthCount }}</p>
          ...
          <div class="bg-gradient-to-br from-gray-50 to-slate-50 p-4 rounded-lg border border-gray-200">
            ...
            <p class="text-3xl font-bold text-gray-900">{{ trainingMonthlyStats.lastMonthCount }}</p>
          ...
          <div class="bg-gradient-to-br from-green-50 to-emerald-50 p-4 rounded-lg border border-green-200">
            ...
            <div v-if="trainingMonthlyStats.trend === 'up'" class="text-sm font-semibold text-green-600 mt-2">
              <svg class="w-5 h-5 inline" fill="currentColor" viewBox="0 0 20 20">
                <path fill-rule="evenodd" d="M5.293 7.707a1 1 0 010-1.414l4-4a1 1 0 011.414 0l4 4a1 1 0 01-1.414 1.414L11 5.414V17a1 1 0 11-2 0V5.414L6.707 7.707a1 1 0 01-1.414 0z" clip-rule="evenodd"/>
              </svg>
              +{{ Math.abs(trainingMonthlyStats.trendPercentage) }}%
            </div>
            <div v-else-if="trainingMonthlyStats.trend === 'down'" class="text-sm font-semibold text-red-600 mt-2">
              <svg class="w-5 h-5 inline" fill="currentColor" viewBox="0 0 20 20">
                <path fill-rule="evenodd" d="M14.707 12.293a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 111.414-1.414L9 14.586V3a1 1 0 012 0v11.586l2.293-2.293a1 1 0 011.414 0z" clip-rule="evenodd"/>
              </svg>
              {{ trainingMonthlyStats.trendPercentage }}%
            </div>
            <div v-else class="text-sm font-semibold text-gray-600 mt-2">
              <svg class="w-5 h-5 inline" fill="currentColor" viewBox="0 0 20 20">
                <path fill-rule="evenodd" d="M3 10a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1z" clip-rule="evenodd"/>
              </svg>
              ±0%
            </div>
            <p class="text-xs text-green-600 mt-1">{{ trainingMonthlyStats.weeklyAverage }}x pro Woche</p>
          </div>
        </div>

        <!-- Heatmap -->
        <div>
          <p class="text-sm font-medium text-gray-700 mb-3">Aktivitätsverlauf</p>
          <div class="overflow-x-auto pb-2">
            <svg :width="trainingHeatmapData.length * 15" :height="7 * 15" class="min-w-full">
              <!-- Draw cells -->
              <g v-for="(week, weekIndex) in trainingHeatmapData" :key="weekIndex">
                <rect
                  v-for="day in week"
                  :key="day.date"
                  :x="weekIndex * 15"
              ...
              <!-- Month labels -->
              <g v-for="(week, weekIndex) in trainingHeatmapData" :key="'label-' + weekIndex">
                <text
                  ...
                  fill="#6b7280"
                >
                  {{ trainingMonthLabels[new Date(week[0].date + 'T12:00:00').getMonth()] }}
                </text>
              </g>
            </svg>
          </div>
